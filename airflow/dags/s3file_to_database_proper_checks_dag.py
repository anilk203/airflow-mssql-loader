...
def download_file_with_checks(**kwargs): 
    print("download_file_with_checks()")
    lc = LoaderUtils()
    directory, working_dir = lc.create_working_dir()  
    file1 = 'file1.csv'
    file_path = directory + "/" + file1
    downloaded_file = lc.download_file_with_checks(file1, file_path)
...
dag = DAG(
    "s3file_to_database_proper_checks",
    description="Pull file from s3 and load into database, Handle different exceptions",
...
t_download_file = PythonOperator(
    ...
    python_callable=download_file_with_checks,
    provide_context=True,
    dag=dag,
)
